...
function createHitTest(particleView, domElement) {
  ...
  function update(scene, camera) {
    ...
    var particleSystem = particleView.particles();
    if (!particleSystem) return;

    raycaster.setFromCamera(mouse, camera);
    var newIntersected = getIntersects(particleSystem);
  ...
  function getIntersects(particleSystem) {
    ...
    if (!octTree) {
      ...
      octTree.init(positions);
    }

    var ray = raycaster.ray;
    var rayOrigin = ray.origin;
    var minDistance = 0;
    var indices = octTree.intersectRay(rayOrigin, ray.direction, minDistance);
    var spherical = [];
    var max = Math.min(indices.length, 4);
    for (var i = 0; i < max; ++i) {
      var idx = indices[i];
      var sphereIntersect = octTree.intersectSphere(positions[idx], positions[idx + 1], positions[idx + 2], 200);
      // note: this is not accurate since we can have duplicates here...
      // Using some sort of hashmap with numberic keys could help
      for (var j = 0; j < sphereIntersect.length ; j++) {
        var candidateIndex =  sphereIntersect[j];
        spherical.push(candidateIndex);
      }
    }

    api.fire('over', spherical, ray);
  }
