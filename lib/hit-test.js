...
function createHitTest(particleView, domElement) {
  ...
  function getIntersects(particleSystem) {
    ...
    for (var i = 0; i < max; ++i) {
      var idx = indices[i];
      var spherIntersects = octTree.intersectSphere(positions[idx], positions[idx + 1], positions[idx + 2], 200);
      for (var j = 0; j < spherIntersects.length ; j++) {
        spherical.push( spherIntersects[j] );
      }
    }

    console.timeEnd('begin');
    api.fire('highlight', spherical, ray);
  }
